...
def plot_concordances_all(files, outname, sources):
	...
	for var in variants:
		...
		for i,source in enumerate(sources):
			...
			axs[0, plot_index].bar(x_values + width*i, concordances, width=width, label=source, color=colors[i])
		...
		for idx, genotype in zip([1,2,3], ['absent, ', 'het, ', 'hom, ']):
			axs[idx, plot_index].set_ylim((0,100))
			axs[idx, plot_index].set_title(genotype + var_to_name[var])
			axs[idx, plot_index].set_xticks(x_values + (width*len(sources)/2)-width/2)
...
#	plt.figure(figsize=(65,5))
	...
	for var in variants:
		...
		for i,source in enumerate(sources):
			...
			is_first = False
			x_values = np.arange(len(samples))
			plt.bar(x_values + width*i, fscores, width=width, label=source, color=colors[i])
		plt.ylim((0.7,1.0))
		plt.title(var_to_name[var])
		plt.xticks(x_values + width*len(sources)/2 - width/2, all_samples, rotation='vertical')
...
#	plt.figure(figsize=(65,5))
	...
	for var in variants:
		...
		for i,source in enumerate(sources):
			...
			is_first = False
			x_values = np.arange(len(samples))
			axs[0, plot_index].bar(x_values + width*i, fscores, width=width, label=source, color=colors[i])
			axs[1, plot_index].bar(x_values + width*i, concordances, width=width, label=source, color=colors[i])
		for idx, metric in zip([0,1], ['adjusted F-score (truvari)', 'gt_concordance [%] (truvari)']):
			if metric == 'adjusted F-score (truvari)':
				axs[idx, plot_index].set_ylim((0.7,1.0))
