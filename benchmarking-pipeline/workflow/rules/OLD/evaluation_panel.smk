...
rule evaluation_panel_plot_missedAFs:
	...
	output:
		"{results}/leave-one-out/pangenie/plots/afs-missed-alleles.pdf"
	resources:
		mem_mb = 10000
	conda:
		"../envs/pandas.yml"
...
rule evaluation_panel_collect_recomb:
	...
	"""
	input:
		expand("{{results}}/leave-one-out/pangenie/sampled-{{size}}/{{sample}}/{{sample}}-pangenie_multi_paths_{chromosome}.tsv", chromosome = CHROMOSOMES),
	output:
...
rule evaluation_panel_plot_recombination:
	...
	output:
		pdf = "{results}/leave-one-out/pangenie/plots/recombination-stats.pdf"
	resources:
		mem_mb = 1000
	conda:
		"../envs/pandas.yml"
