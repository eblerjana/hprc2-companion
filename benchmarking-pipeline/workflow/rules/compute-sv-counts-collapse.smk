callsets = [c for c in POPULATION_GENOTYPES.keys()]
...
rule prepare_vcf:
	input:
		lambda wildcards: POPULATION_GENOTYPES[wildcards.source]['vcf']
	output:
...
rule extract_intersection_samples_collapse:
	input:
		vcf= lambda wildcards: POPULATION_GENOTYPES[wildcards.source]['vcf'],
		samples = "{results}/samples-intersection.tsv",
		reference = lambda wildcards: POPULATION_GENOTYPES[wildcards.source]['reference'],
		fai = lambda wildcards: POPULATION_GENOTYPES[wildcards.source]['reference'] + ".fai"
	output:
...
rule plot_sv_counts_filtered:
	input:
		raw = lambda wildcards: expand("{{results}}/{source}/vcfs/{source}_intersection_full_raw.vcf.gz", source = [s for s in callsets if not POPULATION_GENOTYPES[s]['collapse']]),
		collapse = lambda wildcards: expand("{{results}}/{source}/vcfs/{source}_intersection_full_collapsed.vcf.gz", source = [s for s in callsets if POPULATION_GENOTYPES[s]['collapse']]),
	...
	conda:
		"../envs/plotting.yml"
	params:
		names_raw = ' '.join([s for s in callsets if not POPULATION_GENOTYPES[s]['collapse']]),
		names_collapsed = ' '.join([s for s in callsets if POPULATION_GENOTYPES[s]['collapse']]),
