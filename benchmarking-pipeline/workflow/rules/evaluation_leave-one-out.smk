...
rule evaluation_prepare_truth:
	...
	"""
	input:
		"{results}/leave-one-out/preprocessed-vcfs/{sample}_bi_no-missing.vcf.gz"
	output:
		temp("{results}/leave-one-out/truth/{sample}-truth.vcf")
	conda:
		"../envs/genotyping.yml"
	priority: 1
	resources:
		mem_mb=20000
	log:
		"{results}/leave-one-out/truth/{sample}-truth.log"
	shell:
...
rule evaluation_untypable_ids:
	...
	input:
		PANEL_BI
	output:
		lists = "{results}/leave-one-out/untypable-ids/{sample}-untypable.tsv",
		summary = temp("{results}/leave-one-out/untypable-ids-{sample}.tsv")
	params:
		out = "{results}/leave-one-out/untypable-ids"
	conda:
		"../envs/genotyping.yml"
	shell:
...
rule evaluation_generally_untypable_ids:
	...
	resources:
		mem_mb = 50000
	log:
		"{results}/leave-one-out/untypable-ids/{sample}-untypable-all.log"
	shell:
...
rule evaluation_vcfeval:
	"""
	Use vcfeval to compute precision + recall statistics for evaluation.
	"""
	input:
		callset = "{results}/leave-one-out/{method}/{sample}/{sample}_{method}_bi_genotyping-typable-{vartype}.vcf.gz",
		callset_tbi = "{results}/leave-one-out/{method}/{sample}/{sample}_{method}_bi_genotyping-typable-{vartype}.vcf.gz.tbi",
		baseline = "{results}/leave-one-out/truth/{sample}-truth-typable-{vartype}.vcf.gz",
		baseline_tbi = "{results}/leave-one-out/truth/{sample}-truth-typable-{vartype}.vcf.gz.tbi",
		regions = region_to_bed,
		sdf = "{results}/leave-one-out/SDF"
	output:
		"{results}/leave-one-out/{method}/{sample}/vcfeval-typable/{regions}_{vartype}/summary.txt"
	conda:
		"../envs/genotyping.yml"
	priority: 1
	wildcard_constraints:
		...
		vartype = "|".join(ALLOWED_VARIANTS)
	params:
		tmp = "{results}/leave-one-out/{method}/{sample}/vcfeval-typable/{regions}_{vartype}_tmp",
		outname = "{results}/leave-one-out/{method}/{sample}/vcfeval-typable/{regions}_{vartype}",
		which = "--all-records"
	resources:
		mem_mb = 30000,
		walltime = "1:40:00"
...
rule evaluation_truvari:
	"""
	Use truvari to compute precision + recall statistics for evaluation.
	"""
	input:
		callset = "{results}/leave-one-out/{method}/{sample}/{sample}_{method}_bi_genotyping-typable-{vartype}.vcf.gz",
		callset_tbi = "{results}/leave-one-out/{method}/{sample}/{sample}_{method}_bi_genotyping-typable-{vartype}.vcf.gz.tbi",
		...
		reference = REFERENCE
	output: 
		"{results}/leave-one-out/{method}/{sample}/truvari-typable/{regions}_{vartype}/summary.txt"
	conda:
		"../envs/truvari.yml"
	priority: 1
	wildcard_constraints:
...
rule evaluation_collect_typed_variants:
	...
	"""
	input:
		callset = "{results}/leave-one-out/preprocessed-vcfs/{sample}_bi_no-missing.vcf.gz",
		regions= region_to_bed,
		ids="{results}/leave-one-out/untypable-ids/{sample}-untypable-all.tsv"
	output:
		"{results}/leave-one-out/genotyped-ids/{sample}_{regions}_{vartype}.tsv"
	conda:
		"../envs/genotyping.yml"
	wildcard_constraints:
		sample = "|".join(LEAVE_ONE_OUT_SAMPLES),
...
rule evaluation_genotype_concordances:
	...
	input:
		...
		baseline_tbi =  "{results}/leave-one-out/truth/{sample}-truth-typable-{vartype}.vcf.gz.tbi",
		regions = region_to_bed,
		typed_ids = "{results}/leave-one-out/genotyped-ids/{sample}_{regions}_{vartype}.tsv",
	output:
		tmp_vcf1 = temp("{results}/leave-one-out/{method}/{sample}/concordance/{regions}_{vartype}_base.vcf"),
		tmp_vcf2 = temp("{results}/leave-one-out/{method}/{sample}/concordance/{regions}_{vartype}_call.vcf"),
		summary = "{results}/leave-one-out/{method}/{sample}/concordance/{regions}_{vartype}/summary.txt"
	conda:
		"../envs/genotyping.yml"
	wildcard_constraints:
		sample = "|".join(LEAVE_ONE_OUT_SAMPLES),
		vartype = "|".join(ALLOWED_VARIANTS)
	log:
		"{results}/leave-one-out/{method}/{sample}/concordance/{regions}_{vartype}/summary.log"
	resources:
		mem_mb = 40000,
		walltime = "0:40:00"
	priority: 1
...
rule evaluation_collect_results:
	...
	"""
	input:
		lambda wildcards: expand("{{results}}/leave-one-out/{{method}}/{sample}/{{metric}}/{{regions}}_{{vartype}}/summary.txt", sample = LEAVE_ONE_OUT_SAMPLES),
	output:
		"{results}/leave-one-out/plots/{metric}/{metric}_{method}_{regions}_{vartype}.tsv"
	priority: 1
	shell:
		"""
...
rule plot_resources:
	...
	conda:
		"../envs/genotyping.yml"
	params:
		outname = "{results}/leave-one-out/plots/resources"
	shell:
		"""
