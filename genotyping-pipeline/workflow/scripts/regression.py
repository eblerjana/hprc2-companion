...
def plot(df, variant_types, metrics, outname):
...
if __name__ == "__main__":
	parser = argparse.ArgumentParser(prog='regression.py', description=__doc__)
	...
	parser.add_argument('--plot-only', action='store_true', default=False, help="Table is <outname>_regression.tsv computed by this script. Plot only.")
	parser.add_argument('--regression-only', action='store_true', default=False, help="Only perform regression and do not plot.")
	args = parser.parse_args()

	# if neither --plot-only nor --regression-only is set, perform full pipeline
	perform_plotting = args.plot_only
	perform_regression = args.regression_only

	if not args.plot_only and not args.regression_only:
	...
	if perform_regression:
		...
		# perform regression for large variants and store regression scores.
		...
		df = df.assign(self_fail = lambda df: (df['pangenie_self-genotyping_correct [%]'] < 90.0) & (df['pangenie_self-genotyping_considered_samples'] > 0) )
	...
	#			regressor = GradientBoostingRegressor(n_estimators=20, random_state=0)
		...
		df.to_csv(outname + '_regression.tsv', sep='\t', index=False, na_rep='nan')	

	if perform_plotting:
		# create plots for unfiltered / filtered sets
		plot(df, variant_types, metrics, outname)
