...
rule polish_align_hifi:
	"""
	Align Hifi reads to consensus haplotype.
	"""
	input:
	...
	output:
		"{results}/polishing/{callset}/hifi/hifi_{sample}_{haplotype}.bam"
	...
	conda:
		"../envs/minimap.yml"
	log:
		mm2 = "{results}/polishing/{callset}/hifi/hifi_{sample}_{haplotype}.log",
		sam = "{results}/polishing/{callset}/hifi/hifi_{sample}_{haplotype}.sam.log"
	benchmark:
		"{results}/polishing/{callset}/hifi/hifi_{sample}_{haplotype}.benchmark.txt"
	threads: 24
	params:
		sam_threads = 8
	shell:
		"minimap2 -ax map-hifi -Y --MD --eqx -t {threads} --secondary=no {input.haplotype} {input.reads} 2> {log.mm2} "
		" | "
...
rule polish_call_small_variants:
	...
	"""
	input:
		bam = "{results}/polishing/{callset}/hifi/hifi_{sample}_{haplotype}.bam",
